...
import com.komy.coursecatalogservice.exception.InstructorNotFoundException
import com.komy.coursecatalogservice.repository.CourseRepository
import mu.KLogging
import org.springframework.stereotype.Service

@Service
class CourseService(private val courseRepository: CourseRepository, private val instructorService: InstructorService) {
    companion object : KLogging()

    fun addCourse(courseDTO: CourseDTO): CourseDTO {
        val instructorOptional = instructorService.findByInstructorId(courseDTO.instructorId!!)
        if (!instructorOptional.isPresent) {
            throw InstructorNotFoundException("Instructor not found for the passed Id: ${courseDTO.instructorId}")
        }
    ...
    fun retrieveAllCourses(courseName: String?): List<CourseDTO> {
        val courses = courseName?.let { courseRepository.findCoursesByName(it) } ?: courseRepository.findAll()
        val courseDTOS = courses.map { CourseDTO(it.id, it.name, it.category) }
        return courseDTOS
    }

    fun retrieveCourseById(id: Int): CourseDTO? {
        return courseRepository.findById(id).map { CourseDTO(it.id, it.name, it.category) }
            .orElse(null)
    }
    ...
    fun updateCourse(id: Int, courseDTO: CourseDTO): CourseDTO {
        ...
        return if (existingCourse.isPresent) {
            existingCourse.get().let {
                ...
                it.instructor = instructorService.findByInstructorId(courseDTO.instructorId!!).get()
